   ...
   internal class libz
    {
        public static object ReadSubKeyValue(string subKey, string keyName)
        {
            ...
            {
                if (rkSubKey == null)
                    throw new Exception(string.Format(@"Error while reading registry key: {0}\{1} does not exist!", subKey, keyName));
                ...
                { 
                    throw new Exception(string.Format("Error while reading registry key: {0} param: {1}. ErrorMessage: {2}", subKey, keyName, ex.Message));
                }
            }
        }
